...
\cite{Nagara}
\cite{Nagara2}
この実験は$\Xi$$^-$粒子を生成するためにDiamond Targetを原子核乾板の上流に設置し、生成された$\Xi$$^-$粒子を原子核乾板に照射した。
\par
この実験により7例のDouble-$\Lambda$Hyper核の崩壊事象を観測し、その中の1例でのみ崩壊モードを一意に決定することができた。
この1例をNAGARA Eventと名付けた。
図を入れる。NagaraとKisoの画像。
...
\par
図\ref{fig:compton_and_cosmicray_in_emulsion}は製造からの時間経過による原子核乾板記録された宇宙線、コンプトン電子の蓄積量を示している。
赤色が岐阜大学内で保管した場合、青色が神岡鉱山内で保管した場合を示している。
